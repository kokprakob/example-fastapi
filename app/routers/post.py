...
from fastapi import FastAPI, HTTPException, Query, Response, status, Depends, APIRouter
...
def get_po1sts(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user),limit: int = 10, skip: int = 0, search: Optional[str] = ""):
